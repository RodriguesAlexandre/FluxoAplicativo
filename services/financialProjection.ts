    ...
    for (const month of sortedMonths) {
        ...
        state.monthlyAdjustments.forEach(adj => {
            if (month >= adj.startMonth && (!adj.endMonth || month <= adj.endMonth)) {
                ...
                const isConfirmedThisMonth = (adj.confirmedMonths || []).includes(month);

                if (adj.type === 'income') {
                    monthIncome += value;
                    if (isConfirmedThisMonth) monthConfirmedIncome += value;
                } else {
                    monthExpense += value;
                    if (isConfirmedThisMonth) monthConfirmedExpense += value;
                }
            }
        });
