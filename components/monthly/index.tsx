    ...
    const handleToggleAdjustmentConfirmation = useCallback((adjustmentId: string) => {
        setState(prev => {
            ...
            const newAdjustments = prev.monthlyAdjustments.map(adj => {
                // Not the item we are looking for, return it as is.
                if (adj.id !== adjustmentId) {
                    return adj;
                }
    
                // This is the item to update.
                const oldConfirmedMonths = adj.confirmedMonths || [];
                const wasConfirmed = oldConfirmedMonths.includes(currentMonth);
    
                const newConfirmedMonths = wasConfirmed
                    ? oldConfirmedMonths.filter(month => month !== currentMonth) // filter creates a new array
                    : [...oldConfirmedMonths, currentMonth]; // spread creates a new array
    
                // Calculate the effect on the checking account balance.
                const sign = adj.type === 'income' ? 1 : -1;
                if (wasConfirmed) {
                    balanceChange = -(adj.value * sign); // It was confirmed, now it's not. Subtract effect from balance.
                } else {
                    balanceChange = adj.value * sign; // It was pending, now it's confirmed. Add effect to balance.
                }
