...
export const MonthlyControlView: React.FC<MonthlyControlViewProps> = ({ state, setState, onOpenOracle, isSimulating, onStartSimulation, onSaveChanges, onDiscardChanges }) => {
    ...
    const handleToggleAdjustmentConfirmation = useCallback((adjustmentId: string) => {
        setState(prev => {
            ...
            const adjIndex = prev.monthlyAdjustments.findIndex(a => a.id === adjustmentId);
            if (adjIndex === -1) return prev;
    
            // 1. Isolate the original object.
            const originalAdj = prev.monthlyAdjustments[adjIndex];
    
            // 2. Determine the new state for confirmedMonths.
            const oldConfirmedMonths = originalAdj.confirmedMonths || [];
            const wasConfirmed = oldConfirmedMonths.includes(currentMonth);
            const newConfirmedMonths = wasConfirmed
                ? oldConfirmedMonths.filter(m => m !== currentMonth)
                : [...oldConfirmedMonths, currentMonth];
    
            // 3. Create a completely new adjustment object.
            const updatedAdj = {
                ...originalAdj,
                confirmedMonths: newConfirmedMonths,
            };
    
            // 4. Create a new array for all adjustments, replacing the old one.
            const newAdjustments = [
                ...prev.monthlyAdjustments.slice(0, adjIndex),
                updatedAdj,
                ...prev.monthlyAdjustments.slice(adjIndex + 1),
            ];
    
            // 5. Calculate balance change.
            let balanceChange = 0;
            const sign = originalAdj.type === 'income' ? 1 : -1;
            balanceChange = wasConfirmed
                ? -(originalAdj.value * sign) // Reverting a confirmation
                : (originalAdj.value * sign);  // Adding a confirmation
    ...
    const handleDrop = (e: React.DragEvent, dropTargetEntry: UnifiedEntry, listType: 'income' | 'expense') => {
        e.preventDefault();
        if (!draggedEntry || draggedEntry.id === dropTargetEntry.id || draggedEntry.item.type !== dropTargetEntry.item.type) {
            handleDragEnd();
            return;
        }
        ...
        const targetIndex = reorderedList.findIndex(item => item.id === dropTargetEntry.id);
        
        const [movedItem] = reorderedList.splice(draggedIndex, 1);
        reorderedList.splice(targetIndex, 0, movedItem);
    
        // Update state with new orders, ensuring immutability
        setState(prev => {
            if (!prev) return prev;
            
            const orderMap = new Map<string, number>();
            reorderedList.forEach((entry, index) => {
                orderMap.set(entry.id, index);
            });
    
            const newCategories = prev.categories.map(cat => 
                orderMap.has(cat.id) ? { ...cat, order: orderMap.get(cat.id)! } : cat
            );
            const newAdjustments = prev.monthlyAdjustments.map(adj => 
                orderMap.has(adj.id) ? { ...adj, order: orderMap.get(adj.id)! } : adj
            );
